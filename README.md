...
It defines the Summarizer’s greeting rules, tone, required user inputs, boundaries (no hallucinations, no invented citations), output structure (paper summary, section table, expert summary, lay summary, glossary), and all PS2-derived constraints.
...
Open system_prompt.md.
Copy the full content into an LLM (e.g., Microsoft Copilot or ChatGPT) as the system prompt.
Provide:
the paper text or chunks
your section list
your target audience (expert, semi-technical, layperson)
The model will process the paper through all modules and generate:
an overall summary
